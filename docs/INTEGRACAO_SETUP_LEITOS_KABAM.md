# Integração entre Setup de Leitos e Kabam

## 1. Objetivo

Este documento apresenta uma proposta para integrar o **Setup de Leitos** ao
**Kabam**, permitindo que os dois sistemas compartilhem informações sem
...
cada leito. O Kabam poderá controlar tarefas, pendências, responsáveis, prazos e
...
Desocupado
   |
   v
Kabam acompanha higienização ou manutenção
   |
   v
Apto
   |
...
Inicialmente, a conclusão de uma tarefa no Kabam deve apenas informar ao Setup
...
O Kabam consulta a API do Setup em intervalos regulares para identificar
...
de status ou de bloqueio, o Setup envia um evento ao Kabam. A informação chega
...
O campo `event_id` deve ser único. O Kabam deve armazená-lo para que o mesmo
evento, caso seja reenviado, não crie tarefas duplicadas.

## 6. Informações devolvidas pelo Kabam

O Kabam poderá disponibilizar ou enviar ao Setup:
...
- observação curta;
- link para abertura da tarefa no Kabam.
...
- uma indisponibilidade do Kabam não pode impedir o registro do leito;
- uma indisponibilidade do Setup não deve apagar tarefas do Kabam;
...
exemplo, o Setup é responsável pelo status do leito e o Kabam é responsável
pela situação da tarefa.

Quando o Kabam informar que uma tarefa foi concluída, o Setup não deve enviar
...
- permitir que o Kabam associe tarefas a um identificador de leito;
...
- qual problema e quais tarefas o Kabam controlará inicialmente;
...
2. o Kabam cria ou movimenta tarefas automaticamente;
3. o Kabam devolve `aberta`, `em andamento`, `concluída` ou `com pendência`;
